...
const LETTERS = ["A", "B", "C", "D"]; // SAT uses A-D for every question
...
/**
 * SAT Reading & Writing runner: a series of independent items, one per screen.
 * Each item has its own short text (left) and a single question (right).
 *
 * Props:
 *  - writing:  { questions: [{ category, text, prompt, choices:[string], answerIndex, explanation }] }
 *  - test:     "SAT"
 *  - source:   "ai" | "sample"
 *  - verified: whether the 2nd-model verification pass approved the set
 *  - onExit(), onComplete({ score, total })
 */
export default function WritingRunner({ writing, test, source, verified, onExit, onComplete }) {
  const questions = writing.questions;
        ...
        <div className="lg:sticky lg:top-28 lg:self-start">
          <div className="glass p-6 sm:p-7">
            <p className="flex items-center gap-2 font-display text-xs font-bold tracking-widest text-cyan-300 uppercase">
              ...
              {question.category || "Reading & Writing"}
            </p>
            <p className="mt-4 leading-relaxed whitespace-pre-line text-slate-200">
              {question.text}
            </p>
          </div>
        </div>
